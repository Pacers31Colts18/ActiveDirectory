...
$DEPTGroups = Get-ADGroup -Filter {name -like "IU-IUSM-RSCH-File-IAUC-File-Paper-Approved*"} -Properties * | Sort-Object | Select Name,Description

foreach ($DG in $DEPTGroups) {
    $DGmembers = Get-ADGroupMember -Identity $DG.name
    "<----------------------------->"
    $DG.name
    $DG.description
    $DGmembers.name
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUt85oPxeW152VQqQkyhjvNNvT
# E7qgggt5MIIFhjCCBG6gAwIBAgIQNa8HWQ8ES6XAd67RtM/zIDANBgkqhkiG9w0B
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFJFF
# TQYoXapgQtQcHFkomHQF8wz5MA0GCSqGSIb3DQEBAQUABIIBAHuurGKy8DdmnCBH
# aRJBZEJbak+6aMa1U+sypA4cjEo0QRW/GyBCwTyFPdka78X72FskGGUThh1AooiZ
# OZFUqXG84Ea4pXc6lGpMP2dHdu7IpdlZvGCJgiRlHEqITMXHnhVoV1gY01B4KiYW
# wWMMwpQC7pSz12kIFsGFafSX0wuf+e/4s5VqduBemmzvcrXzebBIHIyreNYNzGfg
# cIchU5q1QsllWxFJb9giHTHaSxT84n4tmm+5lbEYSKDLjPH+sCqKwBgFBnOnryYl
# EoDDVEHC6iscyVywu5o62zv9hafS0mJbWd7qVahbZITxvBSGRJp0FwzqwgA62ror
# RinQkhk=
