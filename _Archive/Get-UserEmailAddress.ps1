Function Get-UserEmailAddress {
    ...
    $ErrorActionPreference = 1
        ...
        Foreach ($Username in $LookupUsers)
            {
              $EmailAddress = (Get-ADUser -Identity $Username -Properties emailaddress).emailaddress
              ...
              $contactObject | Add-Member -MemberType NoteProperty -Name "Email" -Value $EmailAddress
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUjM/lasCjmOeCdzjNe0k10V+e
# Dj+gggt5MIIFhjCCBG6gAwIBAgIQNa8HWQ8ES6XAd67RtM/zIDANBgkqhkiG9w0B
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFOAx
# 6dI8KPN8su5YAk1krJP9CNvGMA0GCSqGSIb3DQEBAQUABIIBABYi45V0U7pgu4A3
# xIIJtKihAWs84rPET09bpq+jqkutzb4ziJvg2FKQOU+bsgmdstzbavdXsZy305uE
# jk76UujiPte8uAhAOeasu+BiUzqnR2x5ag7jTehmZWAIV2cE4Am0PjoAdKfzMEPK
# nVVe0nMxmGK2pS0ZQzenzvyhLvS6OdmX+7k31wOVe5cmNukKbFUoH9nDgr5rr4iG
# k6wwezZFCsSQijukzmYE1kA8dXaRpv4kS+wEm18bv0wVRSm7VXWBZAxIHInPm4IB
# tpqzWcC/yn4KLUs+SlQr3nuvkiNhrxqZdUb1ohOcKmuZvAdLaBtqsaKBarJNbasN
# k6dSeFY=
